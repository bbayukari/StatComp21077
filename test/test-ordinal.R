...
set.seed(1)
R <- 1
seed <- sample(1:(14*R),14*R)
seed.idx <- 1

TPR     <- list(length=14)
TNR <- list(length=14)
ReErr <- list(length=14)
SLE <- list(length=14)
BS <- list(length=14)
MR <- list(length=14)
MAE <- list(length=14)
...
n.total <- c(rep(500,8),100,200,400,800,1600,3200)
p.total <- c(seq(20,40,5),500,1500,2500,rep(500,6))

for(test.idx in 1:14){
    n <- n.total[test.idx]
    p <- p.total[test.idx]
    TPR[[test.idx]] <- matrix(0,nrow = 3,ncol = R)
    TNR[[test.idx]] <- matrix(0,nrow = 3,ncol = R)
    ReErr[[test.idx]] <- matrix(0,nrow = 3,ncol = R)
    SLE[[test.idx]] <- matrix(0,nrow = 3,ncol = R)
    BS[[test.idx]] <- matrix(0,nrow = 3,ncol = R)
    MR[[test.idx]] <- matrix(0,nrow = 3,ncol = R)
    MAE[[test.idx]] <- matrix(0,nrow = 3,ncol = R)
    time[[test.idx]] <- matrix(0,nrow = 3,ncol = R)
